...
export default function AssignModal({machines,setMachines,castingTypes,assemblyModels,purchasedComponents,wip,onClose}) {
  ...
  const [jobKind,setJobKind]=useState('assembly'); // only relevant when the selected machine's type is 'assembly'
  const [selCt,setSelCt]=useState('');
  const [selRouteId,setSelRouteId]=useState('');
  const [selNodeId,setSelNodeId]=useState('');
  const [selAssemblyId,setSelAssemblyId]=useState('');
  ...
  // A machine of type 'assembly' can take EITHER an assembly build OR a casting route's final
  // step (e.g. a "Final inspect" node) — the toggle is only shown/meaningful for this machine
  // type; every other machine type is implicitly a casting-route job.
  const isAssemblyMachine=mObj?.type==='assembly';
  const effectiveJobKind=isAssemblyMachine?jobKind:'casting';
  ...
  const assemblyObj=selAssemblyId?(assemblyModels||[]).find(a=>Number(a.id)===Number(selAssemblyId)):null;
  useEffect(()=>{setSelCt('');setSelRouteId('');setSelNodeId('');setSelAssemblyId('');setNeedsSetup(false);setSetupHours(2);setJobKind('assembly');},[selM,selShift]);
  ...
  const assign=async()=>{
    if(effectiveJobKind==='assembly'){
      if(!selM||!selOp||!selAssemblyId) return setMsg('Select machine, operator, and assembly model.');
      const opUser=users.find(u=>u.username===selOp);
      setSubmitting(true);
      try{
        const updated=patchMachineShift(machines,selM,selShift,{
          castingTypeId:null,routeId:null,nodeId:null,assemblyModelId:Number(selAssemblyId),
        ...
        setMsg('Assigned!'); setSelM(''); setSelOp(''); setSelAssemblyId('');
      }finally{
        setSubmitting(false);
      }
      return;
    ...
    try{
      const updated=patchMachineShift(machines,selM,selShift,{
        castingTypeId:Number(selCt),routeId:Number(selRouteId),nodeId:Number(selNodeId),assemblyModelId:null,prodCount:0,newPieces:0,reworkPieces:0,castingDefects:0,machiningDefects:0,shiftComplete:false,
        assignedOperator:selOp,operator:opUser?opUser.name:selOp,status:'running',progress:0,output:0,
  ...
  return (
    <Modal onClose={onClose} title="Assign jobs" wide>
      <p className="modal-note">Assign an operator to any machine for one step of a chosen route (or, for assembly stations, an assembly build). For CNC/VMC machines, day and night shift assignments are kept separate — select the shift first. Manual machines run a single shift only, so the shift toggle below only applies to CNC/VMC.</p>
      ...
      {isAssemblyMachine&&(
        <div className="field"><label>Job type</label>
          <div className="role-chips">
            <div className={`role-chip${jobKind==='assembly'?' active':''}`} onClick={()=>setJobKind('assembly')}>Assembly build</div>
            <div className={`role-chip${jobKind==='casting'?' active':''}`} onClick={()=>setJobKind('casting')}>Casting route step</div>
          </div>
        </div>
      )}

      {effectiveJobKind==='assembly'?(
        <>
          <div className="field"><label>Assembly model</label>
            <select className="mi" value={selAssemblyId} disabled={!selM} onChange={e=>setSelAssemblyId(e.target.value)}>
              <option value="">— Select assembly model —</option>
              {(assemblyModels||[]).map(a=><option key={a.id} value={a.id}>{a.name} ({a.target}/shift)</option>)}
            </select>
            {selM&&!(assemblyModels||[]).length&&<div style={{fontSize:10,color:'var(--text3)',marginTop:4,fontFamily:'var(--mono)'}}>No assembly models yet — add one via Assembly models</div>}
          </div>
          {assemblyObj&&(
            <div className="info-box neutral" style={{marginBottom:'1rem'}}>
              <div style={{marginBottom:4}}>Max buildable right now: <b style={{color:'var(--text)'}}>{maxBuildable(castingTypes,purchasedComponents,wip,assemblyObj.bom)} {assemblyObj.unit}</b> (target {assemblyObj.target}/shift)</div>
              {assemblyObj.bom.map((line,i)=>{
                const item=(line.kind==='casting'?castingTypes:purchasedComponents).find(x=>Number(x.id)===Number(line.itemId));
                const avail=bomLineAvailable(castingTypes,purchasedComponents,wip,line);
                const buildableFromLine=line.qty>0?Math.floor(avail/line.qty):0;
                const isLimiting=buildableFromLine===maxBuildable(castingTypes,purchasedComponents,wip,assemblyObj.bom);
                return (
                  <div key={i} style={{fontSize:11,color:isLimiting?'var(--warn)':'var(--text2)'}}>
                    {item?item.name:`removed part #${line.itemId}`}: {avail} available · needs {line.qty}/unit{isLimiting?' ← limiting':''}
                  </div>
                );
              })}
            </div>
          ...
          {Object.entries(
            ...
            <div key={opUsername} style={{marginBottom:8}}>
              ...
              {opMachines.map(m=>{
                ...
                const label=job.kind==='assembly'?job.model.name
                  :job.kind==='casting'?`${job.ct.name} — ${job.route?job.route.name:'—'} (${job.stage.name})`
                  :'—';
                return (
                  <div className="pj-row" key={m.id} style={{marginBottom:4}}>
                    <div className="pj-info">
                      <div className="pj-name">{m.id}{m.setupApplied?' · setup applied':''}</div>
                      <div className="pj-meta">{label}{m.setupApplied?` · target ${m.adjustedTarget}`:''}</div>
                    </div>
